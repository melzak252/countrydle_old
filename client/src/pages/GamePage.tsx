  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 px-4">
      <div className="text-center mb-6">
        <h2 className="text-2xl md:text-3xl font-bold mb-1 md:mb-2">{t('gamePage.title')}</h2>
        <p className="text-xs md:text-sm text-zinc-400">{t('gamePage.date', { date: useGameStore.getState().dailyDate })}</p>
      </div>
      ...
      <div className="mb-6 md:mb-8 bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden shadow-lg">
        ...
        >
          <div className="flex items-center gap-2">
            <MapIcon className="text-blue-500 w-[18px] h-[18px] md:w-[20px] md:h-[20px]" />
            <span className="font-bold text-base md:text-lg">{t('gamePage.map')}</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8">
              ...
              <div className={`text-center p-6 md:p-8 rounded-xl ${gameState.won ? 'bg-green-900/20 border border-green-500/50' : 'bg-red-900/20 border border-red-500/50'}`}>
                ...
                <p className="text-lg md:text-xl mb-3 md:mb-4">
                 ...
                 {(correctCountry || guesses.find(g => g.answer)?.guess) && (
                    <p className="text-base md:text-lg">{t('gamePage.answer')} <span className="font-bold">{correctCountry?.name || guesses.find(g => g.answer)?.guess}</span></p>
                 )}
              </div>
            )}
        ...
        <div className="lg:col-span-4 space-y-4 md:space-y-6">
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 md:p-6 shadow-md">
               <h3 className="font-bold text-zinc-400 uppercase tracking-wider mb-4 text-[10px] md:text-sm">{t('gamePage.status')}</h3>
               <div className="grid grid-cols-2 gap-3 md:gap-4">
                  <div className="text-center p-2 md:p-3 bg-zinc-800/50 rounded-lg">
                    ...
                    <div className="text-[10px] md:text-xs text-zinc-500 uppercase leading-tight">{t('gamePage.questionsLeft')}</div>
                  </div>
                  <div className="text-center p-2 md:p-3 bg-zinc-800/50 rounded-lg">
                    <div className="text-xl md:text-2xl font-bold text-teal-500 mb-0.5 md:mb-1">
                      {gameState.remaining_guesses}
                    </div>
                    <div className="text-[10px] md:text-xs text-zinc-500 uppercase leading-tight">{t('gamePage.guessesLeft')}</div>
                  </div>
               </div>
            </div>
            ...
            {guesses.length > 0 && (
                <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-4 md:p-6 shadow-md">
                    <h3 className="font-bold text-zinc-400 uppercase tracking-wider mb-4 text-[10px] md:text-sm">{t('gamePage.yourGuesses')}</h3>
                    <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1 custom-scrollbar">
                        {guesses.map((g) => (
                            <div key={g.id} className={`flex items-center justify-between p-2 md:p-3 rounded-lg border ${g.answer ? 'bg-green-900/20 border-green-500/50 text-green-400' : 'bg-red-900/20 border-red-500/50 text-red-400'} text-sm md:text-base`}>
                                ...
                                {g.answer ? <span className="text-green-500 shrink-0 text-xs md:text-sm font-bold">{t('gamePage.correct')}</span> : <span className="text-red-500 shrink-0 text-xs md:text-sm font-bold">{t('gamePage.incorrect')}</span>}
                            </div>
                        ))}
                    </div>
                </div>
